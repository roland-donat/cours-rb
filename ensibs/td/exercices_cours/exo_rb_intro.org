...
1. Soit $X_{1}, \ldots, X_{n}$ une suite de variables aléatoires (v.a.). En supposant que la loi
   ...
   ? 
2. La complexité d'un réseau bayésien dépend-elle du nombre de liens entre les variables ?
...
2. Oui, plus les variables sont connectées entre elles, plus les lois conditionnelles des variables
...
2. Calculer tous les paramètres de la loi jointe naturelle du modèle.
...
2. Calculer les paramètres de la loi jointe consiste à calculer les probabilités $P(X = x, Y = y, Z
...
1. Écrire l'expression de la loi jointe du modèle. 
2. Donner le nombre de paramètres pour définir la loi jointe représentée par le RB. 
3. Même question sans utiliser la factorisation de la loi jointe associée au RB. 
4. Reprendre les questions 2 et 3 en calculant le nombre de paramètres probabilistes nécessaire à la
   définition de la loi jointe factorisée dans le RB et de la loi jointe naturelle des variables.
...
2. La complexité spatiale du RB correspond à la somme des complexités spatiales des LPC de chaque
   ...
   D'où $CS(RB) = 18$.
3. La complexité spatiale de la loi jointe non factorisée (ou loi jointe naturelle) des v.a. $X_{1},
   X_{2}, X_{3}, X_{4}$ est $CS(LJN) = 2^{4} = 16$
4. Dans le cas du RB, la complexité probabiliste,notée CP correspondant au nombre de paramètres probabilistes
...
5. La technique générale pour répondre aux questions d'indépendance conditionnelle est la suivante :
